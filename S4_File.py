...
def getESS_cet(b,d,IF):
    ...
    '''
    return ESS, SSS, TPR_ESS, PPV_ESS, Power_ESS, P_cet, Income, SS, Profit
...
def Fig1_cet():
    ...
    d = 0.5
    IF = 200
    ESS, SSS, TPR_ESS, PPV_ESS, P_ESS, P_cet, Income, SS, Profit05 = getESS_cet(0.5,d,IF)
    ax1.plot(SS, Income, c='g', label='b=0.5')
    ESS, SSS, TPR_ESS, PPV_ESS, P_ESS, P_cet, Income, SS, Profit02 = getESS_cet(0.2,d,IF)
    ...
    for i, b in enumerate(wB):
        ESS, SSS, TPR_ESS, PPV_ESS, P_ESS, P_cet, Income, SS, Profit = getESS_cet(b,d,IF)
    ...
    ax5.hist(wP_cet, bins=x_bins, weights=incidence, color='r', alpha=0.5)
    ax5.set_xlabel('P(ESS)')
    ax5.set_ylabel('# of studies')
    
    f.savefig('SSG1_cet.pdf')
...
def Fig2_cet():
    ...
    wwwP_cet = np.empty((len(wD),len(wIF),len(wB)))
    for d_idx,d in enumerate(wD):
        for b_idx,b in enumerate(wB):
            for IF_idx,IF in enumerate(wIF):
                wwwESS[d_idx,IF_idx,b_idx], SSS, TPR_ESS, PPV_ESS, wwwP[d_idx,IF_idx,b_idx], wwwP_cet[d_idx,IF_idx,b_idx], I, SS, Pr = getESS_cet(b,d,IF)
    ...
    for d_idx,d in enumerate(wD):   
        for IF_idx,IF in enumerate(wIF):
           ...
           ax.plot(wB, wwwP_cet[d_idx,IF_idx], c='r', label='ESS')
           ax.set_xlim([0,1])
           ax.set_ylim([0,1])
           if d_idx == 0:
               ax.set_title('IF ' + str(IF))
...
def Fig3_cet():
    ...
    for wB_idx, wB in enumerate(wBs):
        ...
        for wIF_idx, wIF in enumerate(wIFs):
            ...
            ax.hist(wP_cet, bins=x_bins_wP, weights=incidence/norm, color='r')
